...
- イベントログはあくまで残存している記録
  ...
  - 侵害者によってログが削除されてしまう場合もある。ランサムウェア事案だと削除が自動化されているケースも多い。
  - ログの改ざん可能性も捨てきれない。EVTXには整合性確認のためのチェックサム機構があるが、暗号学的な改ざん防止機構ではない。単純な削除よりコストが高いので見かけるケースは少ないが。
...
5W1Hでもいいし、[Cyber Kill Chain](https://www.lockheedmartin.com/en-us/capabilities/cyber/cyber-kill-chain.html) でもいいからなんらかのフレームワークに沿って整理するとヌケモレが発生しづらい。
...
見るべきは、不審なログオン、ファイルアクセス、外部通信、リムーバブルメディア接続、管理共有へのアクセス、不審ツールの実行など、漏洩につながる可能性のある行動の痕跡である。
...
[Sysmon](https://learn.microsoft.com/us-en/windows-server/administration/windows-commands/sysmon) があれば細かいイベントも見れるのだが、ほとんどの環境では入っていない。期待するな。
...
### 横展開・リモート操作
- SMBClient.evtx
  - [5140](https://learn.microsoft.com/ja-jp/previous-versions//mt431793(v=vs.85)): ネットワーク共有オブジェクトにアクセス
  - [5142](https://learn.microsoft.com/ja-jp/previous-versions//mt431793(v=vs.85)): ネットワーク共有オブジェクトが追加
  - [5143](https://learn.microsoft.com/ja-jp/previous-versions//mt431793(v=vs.85)): ネットワーク共有オブジェクトが変更
  - [5144](https://learn.microsoft.com/ja-jp/previous-versions//mt431793(v=vs.85)): ネットワーク共有オブジェクトが削除
  - [5168](https://learn.microsoft.com/ja-jp/previous-versions//mt431793(v=vs.85)): SMB/SMB2 の SPN チェックに失敗
...
## 最低限見るべきログ
よく見るのはこのへんかな。  
あとは [AnyDesk](https://anydesk.com/ja) みたいなリモート接続ソフトが入っているならそれ関連のがあることも。
