...
イベントログ調査の目的は、**怪しいイベントIDを網羅的に拾い集めることではなく、対象環境で何が起きたのかを把握し、初動対応や追加調査につなげること** である。  
...
- イベントログは残存しているものしかわからない。
    ...
    - 侵害者によってログが削除されてしまう場合もある。[ランサムウェア事案などでは削除も自動化されているケース](https://www.cybereason.co.jp/blog/threat-analysis-report/10544/)もあり、残存しているログだけではなにもわからないケースもある。
    ...
    - 上記のことから、残存しているイベントログを **単独で事実として扱うのではなく**、他のアーティファクトと突合しながら **侵害者の行動に関する仮説を立て、検証していく** 視点がアナリストにとって重要である。
- ログの保全は真っ先に行う
    - この記事を読んでいる間にも、新しいイベントは記録され、古いイベントは上書きされている。
    - 特に重要な証拠である Security ログは上書きが早く、数日分残っていれば良い方で、数時間分しか残っていないケースもある。([ログオン失敗で埋め尽くされてたりね](https://www.softwareisac.jp/a2ad/log-analysis-4625/))
    - 迷っている暇があったらまず保全。
- 大量のログからどうノイズを減らすか？が非常に重要
    - 1台あたりの容量で、クライアントなら数GB, サーバなら数十～数百GBを見る覚悟がいる。
    - 平時にその機器がどのように使われ、どのようなイベントが記録されるかを把握することがノイズを減らす第一歩である。
...
[5W1H](https://ja.wikipedia.org/wiki/5W1H) でもいいし、[Cyber Kill Chain](https://www.lockheedmartin.com/en-us/capabilities/cyber/cyber-kill-chain.html) でもなんでもいいから、なんらかのフレームワークをベースに整理するとヌケモレが発生しづらい。
...
たとえば **「情報漏洩があったか」を知りたいとして、イベントログに「情報漏洩しました」とは書かれるはずもない。** 見るべきは、不審なログオン、ファイルアクセス、外部通信、リムーバブルメディア接続、管理共有へのアクセス、不審ツールの実行など、**漏洩につながる可能性のある行動の痕跡** である。
...
どう調べればいいかわからなければ、生成AIに聞いてもいい。ただし、ログをそのまま貼るのはやめよう。機微な情報が含まれている可能性があるため、入力する情報は必ずマスキングするなど、所属組織のルールに従うこと。  
また、ちょっと踏み込んだ質問をするなら精度はそんなに高くない (というより、一般論に寄りすぎてしまう) ことは頭の片隅に入れておこう。
...
ログを網羅的に見るのではなく、**とっかかりを見つける** という観点でログを見るのがよい。  
...
- Microsoft-Windows-NTLM%4Operational.evtx
    - [4020](https://support.microsoft.com/en-us/topic/overview-of-ntlm-auditing-enhancements-in-windows-11-version-24h2-and-windows-server-2025-b7ead732-6fc5-46a3-a943-27a4571d9e7b): クライアント - 送信 NTLM 認証の試行
    - [4021](https://support.microsoft.com/en-us/topic/overview-of-ntlm-auditing-enhancements-in-windows-11-version-24h2-and-windows-server-2025-b7ead732-6fc5-46a3-a943-27a4571d9e7b): クライアント - 送信 NTLM 認証の試行
    - [4022](https://support.microsoft.com/en-us/topic/overview-of-ntlm-auditing-enhancements-in-windows-11-version-24h2-and-windows-server-2025-b7ead732-6fc5-46a3-a943-27a4571d9e7b): サーバ - 着信 NTLM 認証の試行
    - [4023](https://support.microsoft.com/en-us/topic/overview-of-ntlm-auditing-enhancements-in-windows-11-version-24h2-and-windows-server-2025-b7ead732-6fc5-46a3-a943-27a4571d9e7b): サーバ - 着信 NTLM 認証の試行
    - [4030](https://support.microsoft.com/en-us/topic/overview-of-ntlm-auditing-enhancements-in-windows-11-version-24h2-and-windows-server-2025-b7ead732-6fc5-46a3-a943-27a4571d9e7b): ドメインコントローラ - 転送された NTLM 認証要求
    - [4031](https://support.microsoft.com/en-us/topic/overview-of-ntlm-auditing-enhancements-in-windows-11-version-24h2-and-windows-server-2025-b7ead732-6fc5-46a3-a943-27a4571d9e7b): ドメインコントローラ - 転送された NTLM 認証要求


### RDP / TerminalServices
RDP接続の試行、認証、セッション開始、切断、再接続など。  
Securityイベントログが消されていても、RDP関連が残っていることが割とある。こっちから流れを追える場合もある。
...
PowerShellの起動、実行内容、スクリプトブロック、ログなど。設定依存なので、記録されていればなるべく見る。  
定期実行されている場合もあるので、普段の運用を把握しつつノイズは頑張って減らす。
...
結構 [悪用される事例](https://www.mbsd.jp/research/20210413/conti-ransomware/) があるので要チェック。
...
- Microsoft-Windows-TaskScheduler%4Operational.evtx
    ...
    - [200](https://mnaoumov.wordpress.com/2014/05/15/task-scheduler-event-ids/): アクションが開始された
    - [201](https://mnaoumov.wordpress.com/2014/05/15/task-scheduler-event-ids/): アクションが完了した
    - [203](https://mnaoumov.wordpress.com/2014/05/15/task-scheduler-event-ids/): アクションの開始に失敗した
...
| [TimeSketch](https://github.com/google/timesketch)  | Google製ログ分析補助ツール。GUIで怪しいやつをフラグ付けとかできる。| plasoと親和性が高い。[使ってみた](https://github.com/google/timesketch) が、複数人で同時作業するとかなら良さがあるのかもしれない。 |
...
Securityログは [JPCERTCC/LogonTracer](https://github.com/jpcertcc/logontracer) より
