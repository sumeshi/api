...
[Win11からは標準搭載（有効化されているとは言ってない）](https://learn.microsoft.com/ja-jp/windows-server/administration/windows-commands/sysmon)
...
## 最低限見るべきログ
とりあえず何でもかんでも保全しておけばよいが、Provider/Channelで件数絞って見たいときにはこのあたりかな。なんにもわからんときは気合の目grep。
...
ファストフォレンジックを見越してデータ取得するなら、[CDIR-Collector](https://github.com/CyberDefenseInstitute/CDIR)あたりをつかっておけばよい。exeカチカチで主要なデータは取れる。  
海外のセキュリティベンダ相手だとなにそれ？ってなることもあるので、何で取得したらよいか聞いておこう。
...
| PowerShell | Windows標準のスクリプト言語。イベントログの抽出・整形に利用できる。           | 一度作っておけば使い回せる。バグがあれば二次被害なので注意。                     |
...
| Excel                   | Microsoftが提供する表計算ソフト。ログの分析や可視化に利用できる。 | 使い慣れている人が多いが、大量ログは読めなかったり、勝手に数値変換されたりする。張り倒すぞ。 |
...
コマンドラインでは次のように複数の処理をチェインさせて遅延評価したり、
```
